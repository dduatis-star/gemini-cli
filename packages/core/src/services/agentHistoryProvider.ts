...
export class AgentHistoryProvider {
  ...
  private async generateIntentSummary(
    ...
    const prompt = `### Strategic Intent Update: Agent Continuity

The conversation history has been truncated. You are generating a strategic "North Star" summary to preserve the agent's continuity and mandate. 

You have these signals to synthesize:
${hasPreviousSummary ? '1. **Previous Summary:** The existing strategic stance before this truncation.\n' : ''}2. **The Action Path:** A chronological "necklace" of tools called in the truncated history: [${actionPath}]
3. **Truncated History:** The specific actions and findings being offloaded now.
...
### Your Goal:
Distill these into a high-density Markdown block that orientates the agent on:
- **Original Mandate:** The ultimate goal (preserve this from the previous summary if available).
- **Current Strategy:** The technical approach currently in play (files, patterns, architectural layers).
- **Evolving Context:** How the plan has shifted based on recent findings or tool outputs.

### Constraints:
- **Format:** Wrap the entire response in <intent_summary> tags.
- **Brevity:** Maximum 15 lines. No filler or conversational preamble.
- **Focus:** The "Why" and "How," not a chronological list.
